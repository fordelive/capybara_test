...
describe 'User' do
  ...
  context 'when logged in with remember_me' do
    ...
    it 'stays logged in when opens Home page' do
      login_page.load
      login_page.log_user_in USER_LOGIN, USER_PASSWORD

      @session_cookies = Capybara.page.driver.browser.manage.all_cookies
      Capybara.current_session.reset_session!

      home_page.load
      home_page.page.driver.browser.manage.delete_all_cookies

      @session_cookies.each {|cookie| home_page.page.driver.browser.manage.add_cookie(cookie)}
      home_page.page.driver.refresh

      expect(home_page).to be_login_successful
    end
  end
  ...
  context 'when logged out' do
    it 'stays logged out when opens Home page' do
      ...
      expect(home_page).to be_logout_successful
    end
  end
end
